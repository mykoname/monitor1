...
# 'уникальность',     'неповторимые',
l_6mark = [   #'Ломаченко', 'Lomachenko'
    'знамя', "флаг"]
mark6 = '|'.join(l_6mark) 
...
df_6=df[(
         (~ (df['Group Name'].str.contains('Думская net')))
               ...
               & (((df['Message'].str.contains(mark6, na=False, regex=True)))
               | ((df['Link Text'].str.contains(mark6, na=False, regex=True)))
               | ((df['Image Text'].str.contains(mark6, na=False, regex=True)))
               | ((df['Link'].str.contains(mark6, na=False, regex=True)))
               | ((df['Description'].str.contains(mark6, na=False, regex=True)))
       )] 

# экспорт в csv:
df_6.to_csv("df_6_2bflag33.csv",encoding='utf-8-sig')
